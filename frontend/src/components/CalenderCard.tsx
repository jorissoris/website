...
export default function CalenderCard(props: any) {
    ...
    const registerButtonInner = (full: boolean) => {
        const button = (inner: string, additionalClasses: string) => <div className={`rounded-md border border-[rgba(1,1,1,0.1)] p-[6px_10px] font-medium cursor-pointer select-none dark:bg-[#121212] dark:border-[rgba(255,255,255,0.3)] ${additionalClasses}`}>{inner}</div>

        return full ? button(text("Full", "Vol"), "bg-lightgray text-[rgba(1,1,1,0.6)] cursor-not-allowed dark:text-[rgba(255,255,255,0.6)]") : button(text("Register", "Aanmelden"), "")
    }
    ...
    const registerButton = props.registerState == 'register' ?
            ...
            : new Date(props.registrationCloseTime) > new Date() ? registerButtonInner(false)
        ...
        : props.registerState == 'full' ? registerButtonInner(true)
    ...
    return <div className="w-full bg-inherit rounded-md border border-[rgba(1,1,1,0.1)] overflow-hidden bg-[#121212] dark:border-[rgba(255,255,255,0.1)] flex flex-col">
        ...
        <div className="p-5 flex justify-between items-center basis-auto grow">
            {props.registerState != 'no-register' ? <div className="flex items-center gap-1.25 mr-5"><GroupIcon className='mr-1.5' /> {props.registrations}</div>: <div></div>}
            {registerButton}
        </div>
    </div>;
}